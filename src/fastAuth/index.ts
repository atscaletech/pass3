...
export const createKey = async (username: string): Promise<GetKeyResponse> => {
  ...
  return navigator.credentials
    ...
    .then(async res => {
      ...
      const key = ed.keyFromSecret((await edSha256.digest()) as any);
      console.log({
        result,
        publicKey,
        publicKeyBytes,
...
export const getKeys = async (
  ...
  return navigator.credentials
    ...
    .then(async (response: any) => {
      ...
      const AuthenticatiorDataJSONHash = Buffer.from(
        new Uint8Array(
          base64.toArrayBuffer(
      ...
      const authenticatorAndClientDataJSONHash = Buffer.concat([
        AuthenticatiorDataJSONHash,
        clientDataJSONHash,
      ]);

      const correctPKs = await recoverPublicKey(
        rAndS.children[0].value,
        rAndS.children[1].value,
      ...
      const firstED = ed.keyFromSecret((await firstEdSha256.digest()) as any);
      const secondED = ed.keyFromSecret((await secondEdSha256.digest()) as any);
      const firstKeyPair = KeyPair.fromString(
        baseEncode(
          new Uint8Array(
            Buffer.concat([
              firstED.getSecret(),
              Buffer.from(firstED.getPublic()),
            ]),
          ),
        ),
      );
      const secondKeyPair = KeyPair.fromString(
        baseEncode(
          new Uint8Array(
            Buffer.concat([
              secondED.getSecret(),
              Buffer.from(secondED.getPublic()),
            ]),
          ),
        ),
      );
