...
export const preformatMakeCredReq = (makeCredReq: any) => {
  ...
  return {
    ...
    ...(makeCredReq.excludeCredentials
      ? {
          excludeCredentials: makeCredReq.excludeCredentials.map((e: any) => {
...
export const recoverPublicKey = async (
  ...
  return [
    Buffer.from(
      new Uint8Array(Buffer.from(Q.encode(true, false))).subarray(1, 65),
    ),
    Buffer.from(
      new Uint8Array(Buffer.from(P.encode(true, false))).subarray(1, 65),
    ),
  ];
};
